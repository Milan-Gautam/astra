...
PATTERNS: list[P] = [
    ...
    # ══════════════════════════════════════════════════════════════════════════
    # +50 NEW PATTERNS (validated) — total ~250
    # ══════════════════════════════════════════════════════════════════════════

    # ── Prefix-based / self-contained ─────────────────────────────────────────
    P("Buildkite Token",
      r"(bkua_[a-zA-Z0-9]{40})",
      "confirmed","Buildkite agent API token",["buildkite","ci"],4.0),
    P("Pulumi Access Token",
      r"(pul-[a-zA-Z0-9]{40})",
      "confirmed","Pulumi Cloud access token",["pulumi","devops"],4.0),
    P("Render API Key",
      r"(rnd_[A-Za-z0-9]{32})",
      "confirmed","Render deploy/API key",["render","hosting"],3.5),
    P("Duffel API Token",
    ...
    P("Prefect API Token",
      r"(pnu_[A-Za-z0-9]{36})",
      "confirmed","Prefect Cloud API token",["prefect","devops"],4.0),
    P("Sourcegraph Access Token",
      r"(sgp_[A-Za-z0-9]{40})",
      "confirmed","Sourcegraph personal access token",["sourcegraph"],4.0),
    P("SonarCloud Token",
      r"(sqa_[A-Za-z0-9]{40})",
      "confirmed","SonarCloud / SonarQube user token",["sonarcloud","ci"],4.0),
    P("WakaTime API Key",
      r"(waka_[a-zA-Z0-9]{8}-[a-zA-Z0-9]{4}-[a-zA-Z0-9]{4}-[a-zA-Z0-9]{4}-[a-zA-Z0-9]{12})",
      "confirmed","WakaTime API key",["wakatime"],3.5),
    P("Tavily API Key",
      r"(tvly-[A-Za-z0-9]{32})",
      "confirmed","Tavily AI search API key",["tavily","ai"],4.0),
    P("Stytch Secret",
      r"(secret-(?:live|test)-[A-Za-z0-9\-]{36})",
      "confirmed","Stytch project secret",["stytch","auth"],4.0),
    P("HubSpot PAT",
      r"(pat-(?:na|eu|ap)[0-9]+-[a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12})",
      "confirmed","HubSpot private app access token",["hubspot","crm"]),
    P("Airtable PAT (new format)",
      r"(pat[A-Za-z0-9]{14,22}\.[a-f0-9]{64})",
      "confirmed","Airtable personal access token",["airtable"],4.0),
    P("Contentful PAT",
      r"(CFPAT-[A-Za-z0-9_\-]{40,})",
      "confirmed","Contentful personal access token",["contentful","cms"],4.0),
    P("Atlassian PAT",
      r"(ATATT3x[A-Za-z0-9+/=]{40,})",
      "confirmed","Atlassian / Jira personal access token",["atlassian","jira"]),
    P("Bitbucket HTTP Token",
      r"(BBDC-[A-Za-z0-9]{32,})",
      "confirmed","Bitbucket HTTP access token",["bitbucket"],4.0),
    P("Linear API Key",
      r"(lin_api_[A-Za-z0-9]{30,})",
      "confirmed","Linear API key",["linear"],4.0),
    P("Typeform Token",
      r"(tfp_[A-Za-z0-9]{40,})",
      "confirmed","Typeform personal access token",["typeform"],4.0),
    P("Postman API Key",
      r"(PMAK-[A-Za-z0-9\-]{40,})",
      "confirmed","Postman API key",["postman","devtools"],4.0),
    ...
    P("Databricks Token",
      r"(dapi[a-f0-9]{32})",
      "confirmed","Databricks personal access token",["databricks","cloud"],3.5),
    P("Dynatrace Token",
      r"(dt0[a-z0-9]{2,5}\.[A-Za-z0-9]{8}\.[A-Za-z0-9]{64})",
      "confirmed","Dynatrace API token",["dynatrace","monitoring"],4.0),
    P("Doppler Token",
      r"(dp\.(?:st|ct)\.[A-Za-z0-9.]{30,})",
      "confirmed","Doppler secrets token",["doppler","secrets"],4.0),
    P("PlanetScale Token",
      r"(pscale_tkn_[A-Za-z0-9_]{32,})",
      "confirmed","PlanetScale service token",["planetscale","database"],4.0),
    P("Grafana Service Account Token",
      r"(glsa_[A-Za-z0-9]{32}_[A-Za-z0-9]{8})",
      "confirmed","Grafana service account token",["grafana","monitoring"],4.0),
    P("Grafana Cloud Access Policy",
      r"(glc_eyJ[A-Za-z0-9+/=]{60,})",
      "confirmed","Grafana Cloud access policy token",["grafana","monitoring"],4.0),
    P("Okta SSWS Token",
      r"(SSWS [A-Za-z0-9_\-]{40,})",
      "confirmed","Okta API token (SSWS prefix)",["okta","identity"],4.0),
    ...
    P("HashiCorp Vault Service Token",
      r"(hvs\.[A-Za-z0-9_\-+/=]{50,})",
      "confirmed","HashiCorp Vault service token",["vault","secrets"],4.0),
    P("HashiCorp Vault Batch Token",
      r"(hvb\.[A-Za-z0-9_\-]{40,})",
      "confirmed","HashiCorp Vault batch token",["vault","secrets"],4.0),
    P("RubyGems API Key",
      r"(rubygems_[a-zA-Z0-9]{48})",
      "confirmed","RubyGems API key",["rubygems","devtools"],4.0),

    # ── Payment — additional ──────────────────────────────────────────────────
    P("Flutterwave Secret Key",
      r"(FLWSECK(?:_TEST)?-[a-zA-Z0-9]{32})",
      "confirmed","Flutterwave secret key",["flutterwave","payment"],3.5),
    P("Flutterwave Public Key",
      r"(FLWPUBK(?:_TEST)?-[a-zA-Z0-9]{32})",
      "probable","Flutterwave public key",["flutterwave","payment"],3.5),
    P("Paystack Secret Key",
      r"(sk_(?:live|test)_[A-Za-z0-9]{40})",
      "confirmed","Paystack secret key",["paystack","payment"],4.0),
    P("Razorpay Key ID",
      r"(rzp_(?:live|test)_[A-Za-z0-9]{14})",
      "confirmed","Razorpay key ID",["razorpay","payment"],3.5),
    P("Checkout.com Secret Key",
      r"(sk_(?:prod|sbox)_[A-Za-z0-9]{32,})",
      "confirmed","Checkout.com secret key",["checkout","payment"],4.0),
    P("WooCommerce Consumer Key",
      r"(ck_[a-f0-9]{40})",
      "confirmed","WooCommerce consumer key",["woocommerce","ecommerce"],3.5),
    P("WooCommerce Consumer Secret",
      r"(cs_[a-f0-9]{40})",
      "confirmed","WooCommerce consumer secret",["woocommerce","ecommerce"],3.5),
    P("Shopify Storefront Token",
      r"(shpuf_[A-Za-z0-9]{32})",
      "confirmed","Shopify storefront access token",["shopify","ecommerce"]),
    P("Shopify Partner Token",
      r"(shppa_[A-Za-z0-9]{32})",
      "confirmed","Shopify partner access token",["shopify","ecommerce"]),
    P("DeepL API Key",
      r"([A-Za-z0-9:\-_]{36}:fx)",
      "confirmed","DeepL API key (:fx suffix)",["deepl","ai"],3.5),

    # ── Context-required (SaaS) ───────────────────────────────────────────────
    P("Zendesk API Token",
      r"(?i)zendesk[_\s]?(?:api[_\s]?)?(?:key|token)\s*[=:]\s*['\"`]([A-Za-z0-9]{40})['\"`]",
      "confirmed","Zendesk API token",["zendesk","support"],3.5),
    P("Vonage / Nexmo Key",
      r"(?i)(?:vonage|nexmo)[_\s]?(?:api[_\s]?)?(?:key|secret)\s*[=:]\s*['\"`]([A-Za-z0-9]{8,20})['\"`]",
      "probable","Vonage / Nexmo API key",["vonage","comms"],3.0),
    P("Snyk API Token",
      r"(?i)snyk[_\s]?(?:api[_\s]?)?(?:key|token)\s*[=:]\s*['\"`]([a-f0-9\-]{36})['\"`]",
      "confirmed","Snyk API token",["snyk","security"],3.5),
    P("Fastly API Key",
      r"(?i)fastly[_\s]?(?:api[_\s]?)?(?:key|token)\s*[=:]\s*['\"`]([A-Za-z0-9_\-]{32})['\"`]",
      "confirmed","Fastly API key",["fastly","cdn"],3.5),
    P("Codecov Token",
      r"(?i)codecov[_\s]?(?:api[_\s]?)?token\s*[=:]\s*['\"`]([A-Za-z0-9\-]{36})['\"`]",
      "confirmed","Codecov upload token",["codecov","ci"],3.5),
    P("Netlify API Token",
      r"(?i)netlify[_\s]?(?:api[_\s]?)?(?:key|token)\s*[=:]\s*['\"`]([A-Za-z0-9_\-]{40,})['\"`]",
      "confirmed","Netlify API token",["netlify","hosting"],3.5),
    P("AssemblyAI API Key",
    ...
    P("Klarna API Key",
      ...
      "probable","Klarna API key",["klarna","payment"],3.0),
]
...
def scan_text(source, text, min_sev="info", filter_tags=None):
    ...
    for pat in PATTERNS:
        ...
        for m in pat._c.finditer(text):
            ...
            seen.add(key)
            out.append(Finding(
                source=source, line_no=_lno(offs, match_pos),
