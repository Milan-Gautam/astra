...
=============================================
300+ unique regex patterns. Line-by-line scanning.
...
BANNER = rf"""{C.BOLD}{C.C}
  ...
  / ___ \ ___) || | |  _ <  / ___ \
 /_/   \_\____/ |_| |_| \_\/_/   \_\
{C.RST}{C.X}  secret & credential scanner v1.2{C.RST}
"""
...
def is_fp(val: str) -> bool:
    ...
    if len(v) > 50 and ci > len(v) * 0.1: return True
    return False

# ═══════════════════════════════════════════════════════════════════════════
# 300+ UNIQUE PATTERNS - ALL DEDUPLICATED
# ═══════════════════════════════════════════════════════════════════════════
...
def build_patterns():
    ...
    add(r'(?i)aws_secret_access_key\s*[=:]\s*[\'"]([A-Za-z0-9\/+=]{40})[\'"]', 'AWS Secret Access Key', 'confirmed', ['aws'], 4.5)
    ...
    add(r'([a-z0-9][a-z0-9\-]*\.s3\.amazonaws\.com)', 'AWS S3 Bucket', 'info', ['aws','recon'])
    add(r'([a-z0-9][a-z0-9\-]*\.s3-website[\.-][a-z0-9\-]+\.amazonaws\.com)', 'AWS S3 Website', 'info', ['aws','recon'])
    add(r'([a-z0-9\-]+\.cloudfront\.net)', 'AWS CloudFront', 'info', ['aws','cdn'])
    add(r'([a-z0-9\-]+\.execute-api\.[a-z0-9\-]+\.amazonaws\.com)', 'AWS API Gateway', 'info', ['aws','api'])
    add(r'([a-z0-9\-]+\.elb\.amazonaws\.com)', 'AWS ELB', 'info', ['aws','infra'])
    add(r'([a-z0-9\-]+\.rds\.amazonaws\.com)', 'AWS RDS', 'info', ['aws','database'])
    add(r'([a-z0-9\-]+\.elasticache\.amazonaws\.com)', 'AWS ElastiCache', 'info', ['aws','database'])
    add(r'([a-z0-9\-]+\.redshift\.amazonaws\.com)', 'AWS Redshift', 'info', ['aws','database'])
    ...
    add(r'(?i)azure[_-]?function[_-]?app\s*[=:]\s*[\'"]([a-z0-9\-]{3,32})[\'"]', 'Azure Function App', 'info', ['azure'])
    add(r'[a-z0-9\-_]+\.blob\.core\.windows\.net', 'Azure Blob Storage URL', 'info', ['azure','storage'])
    add(r'[a-z0-9\-_]+\.mysql\.database\.azure\.com', 'Azure MySQL DB', 'info', ['azure','database'])
    add(r'[a-z0-9\-_]+\.postgres\.database\.azure\.com', 'Azure PostgreSQL DB', 'info', ['azure','database'])
    ...
    add(r'(glrt-[A-Za-z0-9_\-]{20,})', 'GitLab Runner Authentication Token', 'confirmed', ['gitlab'])
    ...
    add(r'(?i)database[_-]?url\s*[=:]\s*[\'"]([^\'"]+)[\'"]', 'Database URL (Generic)', 'confirmed', ['database'])
    add(r'(?i)redis[_-]?password\s*[=:]\s*[\'"]([A-Za-z0-9]{16,})[\'"]', 'Redis Password', 'confirmed', ['database','redis'])
    ...
    add(r'postgresql:\/\/[^:]+:[^@]+@[^\s]+\.neon\.tech', 'Neon Serverless Postgres', 'confirmed', ['database','neon'], 2.5)
    ...
    add(r'https:\/\/hooks\.slack\.com\/services\/T[A-Za-z0-9_]+\/B[A-Za-z0-9_]+\/[A-Za-z0-9_]+', 'Slack Incoming Webhook', 'confirmed', ['messaging','slack'])
    ...
    add(r'-----BEGIN PRIVATE KEY-----', 'PKCS#8 Private Key Header', 'confirmed', ['crypto','private-key'])
    add(r'-----BEGIN ENCRYPTED PRIVATE KEY-----', 'Encrypted Private Key Header', 'confirmed', ['crypto','private-key'])
    add(r'(?i)private[_-]?key\s*[=:]\s*[\'"]([A-Za-z0-9_\-\+\/=]{40,})[\'"]', 'Private Key Value', 'confirmed', ['crypto','private-key'], 4.0)
    add(r'(?i)encryption[_-]?key\s*[=:]\s*[\'"]([A-Za-z0-9\+\/=]{32,})[\'"]', 'Encryption Key Value', 'confirmed', ['crypto'], 3.5)
    add(r'(?i)jwt[_-]?secret\s*[=:]\s*[\'"]([A-Za-z0-9_\-!@#$%^&*]{32,})[\'"]', 'JWT Signing Secret', 'confirmed', ['crypto','jwt'], 3.5)
    ...
    add(r'-----BEGIN CERTIFICATE-----', 'X.509 Certificate', 'info', ['crypto','certificate'])
    add(r'-----BEGIN PUBLIC KEY-----', 'Public Key Header', 'info', ['crypto','public-key'])
    
    # ── API Keys & Authentication (18) ───────────────────────────────────
    add(r'(?i)api[_-]?key\s*[=:]\s*[\'"]([A-Za-z0-9_\-\.]{16,})[\'"]', 'Generic API Key', 'confirmed', ['api','api-key'], 3.0)
    add(r'(?i)api[_-]?secret\s*[=:]\s*[\'"]([A-Za-z0-9_\-\.~!@#]{12,})[\'"]', 'Generic API Secret', 'probable', ['api','secret'], 3.5)
    add(r'(?i)api[_-]?token\s*[=:]\s*[\'"]([A-Za-z0-9_\-\.]{20,})[\'"]', 'Generic API Token', 'probable', ['api','token'])
    add(r'(?i)client[_-]?secret\s*[=:]\s*[\'"]([A-Za-z0-9_\-\.~]{20,})[\'"]', 'OAuth Client Secret', 'confirmed', ['auth','oauth'], 3.0)
    add(r'(?i)client[_-]?id\s*[=:]\s*[\'"]([A-Za-z0-9]{16,})[\'"]', 'OAuth Client ID', 'probable', ['auth','oauth'])
    add(r'(?i)access[_-]?token\s*[=:]\s*[\'"]([A-Za-z0-9_\-\.]{20,})[\'"]', 'Access Token', 'confirmed', ['auth','token'], 3.0)
    add(r'(?i)auth[_-]?token\s*[=:]\s*[\'"]([A-Za-z0-9_\-\.]{20,})[\'"]', 'Authentication Token', 'probable', ['auth','token'], 3.0)
    add(r'(?i)bearer\s+([A-Za-z0-9\-\._~\+\/]{30,}=*)', 'Bearer Authorization Token', 'probable', ['auth','token'], 3.5)
    add(r'(?i)Basic\s+([A-Za-z0-9\+\/=]{20,})', 'HTTP Basic Auth Value', 'probable', ['auth','basic'], 3.0)
    add(r'(?i)refresh[_-]?token\s*[=:]\s*[\'"]([A-Za-z0-9_\-\.]{20,})[\'"]', 'Refresh Token', 'confirmed', ['auth','token'])
    add(r'(?i)id[_-]?token\s*[=:]\s*[\'"]([A-Za-z0-9_\-\.]{20,})[\'"]', 'ID Token', 'probable', ['auth','token'])
    add(r'(?i)session[_-]?secret\s*[=:]\s*[\'"]([A-Za-z0-9_\-!@#$%^&*]{32,})[\'"]', 'Session Secret Key', 'probable', ['auth','session'], 3.0)
    add(r'(?i)secret[_-]?key\s*[=:]\s*[\'"]([A-Za-z0-9_\-!@#$%^&*]{32,})[\'"]', 'Generic Secret Key', 'probable', ['auth','secret'], 3.5)
    add(r'(?i)x-api-key\s*[=:]\s*[\'"]([A-Za-z0-9]{20,})[\'"]', 'X-API-Key Header Value', 'confirmed', ['api','api-key'])
    add(r'(?i)authorization\s*[=:]\s*[\'"](Basic|Bearer)\s+([A-Za-z0-9\-_=]+)[\'"]', 'Authorization Header', 'confirmed', ['auth'])
    add(r'PMAK-[A-Za-z0-9\-]{40,}', 'Postman API Key', 'confirmed', ['api','postman','saas'], 4.0)
    add(r'(?i)algolia[_-]?api[_-]?key\s*[=:]\s*[\'"]([A-Za-z0-9]{32})[\'"]', 'Algolia Admin API Key', 'confirmed', ['api','algolia'], 3.5)
    add(r'(?i)firebase[_-]?token\s*[=:]\s*[\'"]([A-Za-z0-9\-_=]+\.[A-Za-z0-9\-_=]+\.[A-Za-z0-9\-_=]+)[\'"]', 'Firebase Auth Token', 'probable', ['auth','firebase'])
    
    # ── Email Services (6) ───────────────────────────────────────────────
    add(r'[a-f0-9]{32}-us[0-9]{1,2}', 'Mailchimp API Key', 'confirmed', ['email','mailchimp'], 3.5)
    add(r're_[A-Za-z0-9_]{24,}', 'Resend API Key', 'confirmed', ['email','resend'], 4.0)
    add(r'(?i)smtp[_-]?password\s*[=:]\s*[\'"]([A-Za-z0-9]{8,})[\'"]', 'SMTP Password', 'confirmed', ['email','smtp'])
    add(r'(?i)sendgrid[_-]?api[_-]?key\s*[=:]\s*[\'"](SG\.[A-Za-z0-9_\-]{40,})[\'"]', 'SendGrid API Key (Context)', 'confirmed', ['email','sendgrid'], 3.5)
    add(r'(?i)mailgun[_-]?api[_-]?key\s*[=:]\s*[\'"](key-[a-f0-9]{32,})[\'"]', 'Mailgun API Key (Context)', 'confirmed', ['email','mailgun'])
    add(r'(?i)mailgun[_-]?domain\s*[=:]\s*[\'"]([a-zA-Z0-9\-\.]+)[\'"]', 'Mailgun Domain', 'info', ['email','mailgun'])
    
    # ── CI/CD Additional (8) ─────────────────────────────────────────────
    add(r'(?i)jenkins[_-]?token\s*[=:]\s*[\'"]([A-Za-z0-9_\-]{32,})[\'"]', 'Jenkins API Token', 'probable', ['ci_cd','jenkins'])
    add(r'(?i)travis[_-]?token\s*[=:]\s*[\'"]([A-Za-z0-9_\-]{20,})[\'"]', 'Travis CI Token', 'probable', ['ci_cd','travis'])
    add(r'(?i)codecov[_-]?token\s*[=:]\s*[\'"]([A-Za-z0-9\-]{36})[\'"]', 'Codecov Upload Token', 'confirmed', ['ci_cd','codecov'])
    add(r'(?i)terraform[_-]?cloud[_-]?token\s*[=:]\s*[\'"]([A-Za-z0-9]{40,})[\'"]', 'Terraform Cloud Token', 'probable', ['ci_cd','terraform'])
    add(r'(?i)teamcity[_-]?token\s*[=:]\s*[\'"]([A-Za-z0-9]{40,})[\'"]', 'TeamCity Token', 'probable', ['ci_cd','teamcity'])
    add(r'(?i)bamboo[_-]?token\s*[=:]\s*[\'"]([A-Za-z0-9]{32,})[\'"]', 'Bamboo API Token', 'probable', ['ci_cd','bamboo'])
    add(r'(?i)bitrise[_-]?api[_-]?key\s*[=:]\s*[\'"]([A-Za-z0-9]{32,})[\'"]', 'Bitrise API Key', 'probable', ['ci_cd','bitrise'])
    add(r'BBDC-[A-Za-z0-9]{32,}', 'Bitbucket HTTP Access Token', 'confirmed', ['ci_cd','bitbucket'], 4.0)
    ...
    # ── Generic High-Entropy (8) ─────────────────────────────────────────
    add(r'(?i)(?:token|key|secret|pass|pwd)\s*[=:]\s*[\'"]([A-Za-z0-9]{32,})[\'"]', 'High-Entropy Token (32+ chars)', 'possible', ['generic'], 4.2)
    add(r'(?i)(?:token|key|secret)\s*[=:]\s*[\'"]([A-Za-z0-9\+\/=]{40,}={0,2})[\'"]', 'Base64 High-Entropy Secret', 'possible', ['generic'], 4.3)
    add(r'(?i)password\s*[=:]\s*[\'"]([^\'"]{8,})[\'"]', 'Password Literal in Code', 'probable', ['generic','password'], 2.8)
    add(r'(?i)passwd\s*[=:]\s*[\'"]([^\'"]{8,})[\'"]', 'Passwd Literal', 'probable', ['generic','password'], 2.8)
    add(r'(?i)pwd\s*[=:]\s*[\'"]([^\'"]{8,})[\'"]', 'PWD Literal', 'probable', ['generic','password'], 2.8)
    add(r'(?i)access[_-]?key\s*[=:]\s*[\'"]([A-Za-z0-9]{16,})[\'"]', 'Generic Access Key', 'probable', ['generic'])
    add(r'(?i)app[_-]?key\s*[=:]\s*[\'"]([A-Za-z0-9_\-]{16,})[\'"]', 'Application Key', 'probable', ['generic'])
    add(r'(?i)admin[_-]?key\s*[=:]\s*[\'"]([A-Za-z0-9_\-]{16,})[\'"]', 'Admin Key', 'probable', ['generic'])
    ...
    # ── Infrastructure (8) ───────────────────────────────────────────────
    add(r'(?i)ftp[_-]?password\s*[=:]\s*[\'"]([A-Za-z0-9]{8,})[\'"]', 'FTP Password', 'confirmed', ['infra','ftp'])
    add(r'(?i)ldap[_-]?password\s*[=:]\s*[\'"]([A-Za-z0-9]{8,})[\'"]', 'LDAP Password', 'confirmed', ['infra','ldap'])
    add(r'(?i)proxy[_-]?url\s*[=:]\s*[\'"](https?:\/\/[^"\']+)[\'"]', 'Proxy URL', 'info', ['infra','proxy'])
    add(r'(?i)internal[_-]?host\s*[=:]\s*[\'"]([^"\']+)[\'"]', 'Internal Hostname', 'info', ['infra','internal'])
    add(r'(?i)backend[_-]?url\s*[=:]\s*[\'"](https?:\/\/[^"\']+)[\'"]', 'Backend Service URL', 'info', ['infra','backend'])
    add(r'(?i)rabbitmq[_-]?password\s*[=:]\s*[\'"]([A-Za-z0-9]{16,})[\'"]', 'RabbitMQ Password', 'confirmed', ['infra','rabbitmq'])
    add(r'(?i)kafka[_-]?password\s*[=:]\s*[\'"]([A-Za-z0-9]{16,})[\'"]', 'Kafka Password', 'confirmed', ['infra','kafka'])
    add(r'(?i)elasticsearch[_-]?password\s*[=:]\s*[\'"]([A-Za-z0-9]{16,})[\'"]', 'Elasticsearch Password', 'confirmed', ['infra','elasticsearch'])
    
    # ── Auth Providers (8) ───────────────────────────────────────────────
    add(r'SSWS\s+[A-Za-z0-9_\-]{40,}', 'Okta SSWS API Token', 'confirmed', ['auth','okta'], 4.0)
    add(r'(?i)auth0[_-]?client[_-]?secret\s*[=:]\s*[\'"]([A-Za-z0-9_\-]{32,})[\'"]', 'Auth0 Client Secret', 'confirmed', ['auth','auth0'], 3.5)
    add(r'(?i)auth0[_-]?domain\s*[=:]\s*[\'"]([a-zA-Z0-9\-]+\.auth0\.com)[\'"]', 'Auth0 Tenant Domain', 'info', ['auth','auth0'])
    add(r'sk_[a-z]+_[A-Za-z0-9]{30,}', 'WorkOS API Key', 'confirmed', ['auth','workos'], 4.0)
    add(r'sk_prod_[A-Za-z0-9]{40,}', 'Liveblocks Production Key', 'confirmed', ['auth','liveblocks'], 4.0)
    add(r'secret-live-[A-Za-z0-9\-]{36}', 'Stytch Live Secret', 'confirmed', ['auth','stytch'], 4.0)
    add(r'(?i)fusionauth[_-]?token\s*[=:]\s*[\'"]([A-Za-z0-9\-_=]{40,})[\'"]', 'FusionAuth Token', 'probable', ['auth','fusionauth'])
    add(r'(?i)ping[_-]?token\s*[=:]\s*[\'"]([A-Za-z0-9\-_=]{40,})[\'"]', 'Ping Identity Token', 'probable', ['auth','ping'])
    
    # ── MORE SaaS (10) ───────────────────────────────────────────────────
    add(r'(?i)jira[_-]?api[_-]?token\s*[=:]\s*[\'"]([A-Za-z0-9]{32,})[\'"]', 'Jira API Token', 'probable', ['saas','jira'])
    add(r'(?i)confluence[_-]?api[_-]?token\s*[=:]\s*[\'"]([A-Za-z0-9]{32,})[\'"]', 'Confluence API Token', 'probable', ['saas','confluence'])
    add(r'(?i)statuspage[_-]?api[_-]?key\s*[=:]\s*[\'"]([A-Za-z0-9_\-]{20,})[\'"]', 'Statuspage API Key', 'probable', ['saas','statuspage'])
    add(r'(?i)fullstory[_-]?api[_-]?key\s*[=:]\s*[\'"]([A-Za-z0-9\/+]{30,})[\'"]', 'FullStory API Key', 'probable', ['saas','fullstory'])
    add(r'(?i)mixpanel[_-]?token\s*[=:]\s*[\'"]([a-f0-9]{32})[\'"]', 'Mixpanel Project Token', 'probable', ['saas','mixpanel'])
    add(r'(?i)heap[_-]?app[_-]?id\s*[=:]\s*[\'"]([0-9]{8,12})[\'"]', 'Heap Analytics App ID', 'info', ['saas','heap'])
    add(r'(?i)pendo[_-]?integration[_-]?key\s*[=:]\s*[\'"]([A-Za-z0-9_\-]{36,})[\'"]', 'Pendo Integration Key', 'probable', ['saas','pendo'])
    add(r'(?i)sonatype[_-]?token\s*[=:]\s*[\'"]([A-Za-z0-9_\-]{32,})[\'"]', 'Sonatype Nexus Token', 'probable', ['saas','sonatype'])
    add(r'sgp_[A-Za-z0-9]{40}', 'Sourcegraph Access Token', 'confirmed', ['saas','sourcegraph'], 4.0)
    add(r'sqa_[A-Za-z0-9]{40}', 'SonarCloud User Token', 'confirmed', ['saas','sonarcloud'], 4.0)
...
class SecretEngine:
    ...
    def scan(self, url, content):
        """LINE BY LINE scanning - goes through every line and checks every pattern"""
        findings = []
        lines = content.split('\n')
    ...
    def run(self, urls):
        ...
        if not self.json_output and not self.quiet:
            print(BANNER)
            print(f"{C.X}  {len(PATTERNS)} patterns | {len(self.compiled)} active | {self.threads} threads{C.RST}\n")
        all_f = []
        queue = list(urls)
        depth = 0
    ...
    def _sum(self, findings, elapsed):
        ...
        return {'urls': self.scanned_count, 'hits': self.hit_count, 'total': self.total,
                'time': round(elapsed,2), 'by_severity': dict(sc)}
...
{C.BOLD}FLAGS:{C.RST}
  ...
  {C.Y}-l, --list{C.RST}      List rules
    ...
    if args.list:
        print(f"\n{C.BOLD}Secret Detection Engine v1.2 — {len(PATTERNS)} Unique Rules{C.RST}\n")
