#!/usr/bin/env python3
"""
astra — Live JS Secret Detection Engine v1.6
=============================================
310+ unique patterns · Line-by-line scanning
Accurate HTTP status codes (HEAD+GET verified) · Rate limiting
...
class C:
    ...
    M = '\033[95m'; CY = '\033[96m'; W = '\033[97m'; X = '\033[90m'
...
# ═══════════════════════════════════════════════════════════════════════════
# 310+ UNIQUE PATTERNS – UNCHANGED FROM USER-PROVIDED SOURCE
# ═══════════════════════════════════════════════════════════════════════════
...
# ═══════════════════════════════════════════════════════════════════════════
# SECRET SCANNER (fixed: thread-safe state, HEAD+GET status verification,
# soft-404 detection, safe regex-group extraction)
# ═══════════════════════════════════════════════════════════════════════════
...
class SecretScanner:
    ...
    # ── FIX: atomic check-and-add, prevents race between threads ─────────
    def _is_scanned(self, url: str) -> bool:
    ...
    def _record_status(self, status: int):
    ...
    # ── FIX: HEAD-then-GET status verification + soft-404 detection ──────
    ...
    def process_url(self, url: str, depth: int = 0):
        ...
        if self.follow_js and depth < self.max_depth:
            for m in re.finditer(r'https?://[^\s"\'<>]+\.js(?:\?[^\s"\'<>]*)?', content, re.I):
                u = m.group(0)
                with self._scanned_lock:
                    if u not in self._scanned: new.add(u)
    ...
    def run(self, urls: List[str]):
        ...
        all_f = []
        queue = list(dict.fromkeys(urls))  # de-dup while preserving order
        depth = 0
        while queue and depth <= self.max_depth:
            discovered = set()
            with ThreadPoolExecutor(max_workers=self.threads) as ex:
                futs = {ex.submit(self.process_url, u, depth): u for u in queue if u not in self._scanned}
            ...
            with self._scanned_lock:
                queue = list(discovered - self._scanned)
            depth += 1
    ...
    # ── FIX: 401/403 now flagged red (security-relevant), 404 dimmed,
    # 429 distinct color, plus new TIMEOUT (-4) label ────────────────────
    def _status_color(self, status: int) -> str:
        if status == 200: return C.G
        if 201 <= status < 300: return C.G
        if 300 <= status < 400: return C.Y
        if status in (401, 403): return C.R
        if status == 404: return C.X
        if status == 429: return C.M
    ...
    def _status_label(self, status: int) -> str:
        labels = {
            -4: 'TIMEOUT', -3: 'DUP', -2: 'ERROR', -1: 'CONN_ERR',
            401: '401 UNAUTH', 403: '403 FORBID', 404: '404 NOT_FND',
            405: '405 NO_HEAD', 429: '429 RATELIM',
        }
    ...
    def _summary_print(self, findings, elapsed):
        ...
        if not findings: print(f"\n{C.G}  ✓ CLEAN{C.RST}")
            ...
            for sev, c in [('confirmed',C.R),('probable',C.Y),('possible',C.B),('info',C.CY)]:
...
def main():
    parser = argparse.ArgumentParser(description='astra v1.6', add_help=False)
    ...
    if args.help:
        print(f"""
{C.BOLD}astra v1.6 — Live JS Secret Detection Engine{C.RST}
    ...
    if args.list:
        ...
        for cat in sorted(cats):
            ...
            for name, sev in sorted(cats[cat]):
                c = {'confirmed': C.R, 'probable': C.Y, 'possible': C.B, 'info': C.CY}.get(sev, C.W)
