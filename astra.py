...
================================
Extracts JS URLs from files/URLs, then fetches and scans each JS file live.
Handles thousands of URLs. Concurrent scanning. No bullshit.
"""
...
def extract_js_urls(content: str, base_url: str = None) -> Set[str]:
    """Extract JavaScript URLs from any content."""
    js_urls = set()
    ...
    # Pattern 3: Plain URLs ending in .js
    for m in re.finditer(r'https?://[^\s"\'<>]+\.js(?:\?[^\s"\'<>]*)?', content, re.I):
        js_urls.add(m.group(0))
    
    # Pattern 4: sourceMappingURL
    for m in re.finditer(r'sourceMappingURL=([^\s"\']+\.map)', content):
        js_urls.add(m.group(1))
    
    # Pattern 5: webpack chunks / lazy loading
    for m in re.finditer(r'["\']([^"\']+(?:chunk|bundle|vendor|app|main|runtime)[^"\']*\.js)["\']', content, re.I):
        js_urls.add(m.group(1))
    
    # Pattern 6: dynamic imports
    for m in re.finditer(r'import\s*\(\s*["\']([^"\']+\.js)["\']', content):
        js_urls.add(m.group(1))
...
class LiveScanner:
    def __init__(self, severity='possible', show_raw=False, verbose=False,
                 ...
                 max_depth=2, follow_js_urls=True):
        ...
        # SSL context that doesn't verify (for CDNs, etc.)
    ...
    def fetch_url(self, url: str) -> Tuple[str, str]:
        ...
        try:
            ...
            with urllib.request.urlopen(req, timeout=self.timeout, context=self.ssl_context) as resp:
                ...
                if 'text' in ct or 'javascript' in ct or 'json' in ct or 'html' in ct or 'xml' in ct:
                    content = resp.read(10 * 1024 * 1024).decode('utf-8', errors='ignore')  # 10MB max
                    return (url, content)
                else:
                    if self.verbose:
                        print(f"{C.Y}[!] Skipping {url} (Content-Type: {ct}){C.RST}")
                    return (url, None)
        except urllib.error.HTTPError as e:
            if self.verbose:
                print(f"{C.R}[✗] HTTP {e.code}: {url}{C.RST}")
            return (url, None)
        except urllib.error.URLError as e:
            if self.verbose:
                print(f"{C.R}[✗] Connection error: {url} - {e.reason}{C.RST}")
            return (url, None)
        except Exception as e:
            if self.verbose:
                print(f"{C.R}[✗] Error: {url} - {e}{C.RST}")
            return (url, None)
    ...
    def scan_url(self, url: str, depth: int = 0) -> Tuple[str, List[Dict], Set[str]]:
        """Scan a single URL. Returns (url, findings, new_js_urls)."""
        ...
        if self.verbose:
            print(f"{C.X}[*] Fetching: {url}{C.RST}")
        ...
        # Extract JS URLs if we should follow them
        new_js_urls = set()
    ...
    def scan_all(self, urls: List[str]):
        """Main method - scan all URLs with thread pool."""
        if not urls:
        ...
        if not self.json_output:
            ...
            print(f"{C.W}[*] Threads: {self.threads}{C.RST}")
            print(f"{C.W}[*] Max Depth: {self.max_depth}{C.RST}")
            print(f"{C.W}[*] Follow JS URLs: {self.follow_js_urls}{C.RST}")
            print(f"{C.W}[*] Severity: {self.severity.upper()}{C.RST}")
            print(f"{C.W}[*] Patterns: {len(self.compiled)}{C.RST}")
            print()
        ...
        while urls_to_scan and current_depth <= self.max_depth:
            if self.verbose:
                print(f"\n{C.BOLD}{C.C}[*] Depth {current_depth}: Scanning {len(urls_to_scan)} URLs{C.RST}")
                ...
                for future in as_completed(futures):
                    try:
                        ...
                        if findings:
                            ...
                            if not self.json_output:
                                self._show_url_findings(url, findings)
                        else:
                            if self.verbose and not self.json_output:
                                print(f"{C.G}[✓] Clean: {url}{C.RST}")
                        
                        new_urls.update(extracted_urls)
                        
                    except Exception as e:
                        if self.verbose:
                            print(f"{C.R}[✗] Error processing {futures[future]}: {e}{C.RST}")
    ...
    def _print_banner(self):
        ...
        print(f"{C.BOLD}{C.C}║   {len(PATTERNS)} Patterns - Live URL Scanning          ║{C.RST}")
        print(f"{C.BOLD}{C.C}╚══════════════════════════════════════════════╝{C.RST}")
    
    def _show_url_findings(self, url, findings):
        sev_c = {'confirmed': C.R, 'probable': C.Y, 'possible': C.B, 'info': C.C}
        
        print(f"\n{C.BOLD}{C.G}┌─ {url[:80]}{C.RST}")
        ...
        for f in findings[:10]:
            ...
            print(f"{c}│  [{f['severity'].upper():10}] L{f['line']:4} | "
                  f"{f['pattern']:35} | {f['value']} {C.X}{tags}{C.RST}")
        
        if len(findings) > 10:
            print(f"{C.X}│  ... and {len(findings) - 10} more{C.RST}")
...
def main():
    parser = argparse.ArgumentParser(
        description='astra - Live JS Secret Hunter',
...
EXAMPLES:
  ...
  # Scan and follow JS URLs found in responses (depth 2)
  python astra.py -u https://example.com/ -d 2
  
  # With 50 threads, showing raw secrets
  python astra.py -f urls.txt -r -t 50
  
  # JSON output
  python astra.py -f urls.txt -j > results.json
  
  # Filter by tags
  python astra.py -u https://example.com/app.js --tags aws,stripe
  ...
  python astra.py -l
    ...
    parser.add_argument('-d', '--depth', type=int, default=1, 
                       help='Max depth to follow JS URLs (default: 1)')
    parser.add_argument('--no-follow', action='store_true', 
                       help='Don\'t follow JS URLs found in responses')
    parser.add_argument('-l', '--list', action='store_true', help='List patterns and exit')
    
    args = parser.parse_args()
    
    if args.list:
        print(f"\n{C.BOLD}Total patterns: {len(PATTERNS)}{C.RST}\n")
                ...
                for line in f:
                    ...
                    if line and not line.startswith('#'):
                        if line.startswith('http'):
                            ...
                            urls.append('https://' + line if not line.startswith('//') else 'https:' + line)
        except Exception as e:
            print(f"{C.R}[✗] Error reading file: {e}{C.RST}", file=sys.stderr)
            sys.exit(1)
    ...
    if not urls:
        print(f"{C.R}[✗] No URLs provided. Use -u, -f, or pipe URLs via stdin{C.RST}", file=sys.stderr)
        parser.print_help()
        sys.exit(1)
    
    print(f"{C.W}[*] Loaded {len(urls)} URL(s){C.RST}")
