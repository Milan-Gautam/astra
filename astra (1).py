...
PATTERNS: list[P] = [
    ...
    P("AWS ARN",
      r"(arn:aws[a-z\-]*:[a-z0-9\-]+:[a-z0-9\-]*:[0-9]{0,12}:[^\s\"'`<>]{4,})",
      "info","AWS resource ARN (may expose account IDs or resource names)",["aws","recon"]),

    # ══════════════════════════════════════════════════════════
    # GOOGLE / GCP
    # ══════════════════════════════════════════════════════════
    P("Google API / Gemini Key",
      r"(AIza[0-9A-Za-z\-_]{35})",
      "confirmed","Google API key (Maps, Gemini, Vision, etc.)",["google","cloud","ai"]),
    ...
    P("Firebase Realtime DB URL",
      r"(https://[a-z0-9\-]+\.firebaseio\.com)",
      "info","Firebase Realtime Database URL (may be publicly accessible)",["firebase","recon"]),
    P("Firebase Storage Bucket",
      r"(gs://[a-z0-9\-]+\.appspot\.com[^\s\"'`<>]*)",
      "info","Firebase/GCS storage bucket URL",["firebase","recon"]),
    ...
    P("Azure OpenAI Endpoint",
      ...
      "probable","Azure OpenAI deployment endpoint",["azure","cloud","ai"]),
    ...
    P("GitHub Classic Token (hex)",
      r"""(?i)(?:github|gh)[_\s]?(?:api[_\s]?)?(?:token|key)\s*[=:]\s*['"`]([a-f0-9]{40})['"`]""",
      "probable","GitHub classic 40-char hex token",["github"],3.5),
    ...
    P("Bitbucket Token",
      r"(BBDC-[A-Za-z0-9]{32,})",
      "confirmed","Bitbucket HTTP access token",["bitbucket"]),
    P("Atlassian PAT",
      r"(ATATT3x[A-Za-z0-9+/=]{40,})",
      "confirmed","Atlassian/Jira personal access token",["atlassian","jira"]),
    ...
    P("Twilio Auth URL",
      r"(https://[A-Za-z0-9]+:[A-Za-z0-9]+@api\.twilio\.com)",
      "confirmed","Twilio credentials embedded in API URL",["twilio"]),
    ...
    P("Stripe Live Publishable Key",
      r"(pk_live_[0-9a-zA-Z]{24,99})",
      "probable","Stripe live publishable key (exposed in frontend — note the env)",["stripe","payment"]),
    P("Stripe Test Keys",
    ...
    P("Okta SSWS Token",
      r"(SSWS\s+[A-Za-z0-9_\-]{40,})",
      "confirmed","Okta API token (SSWS prefix)",["okta","identity"]),
    P("Okta Domain",
      r"([a-zA-Z0-9\-]+\.okta\.com)",
      "info","Okta domain (reveals org identity)",["okta","recon"]),
    P("Salesforce Token",
      r"(00D[A-Za-z0-9]{12,18}![A-Za-z0-9_.]{10,})",
      "confirmed","Salesforce session/access token",["salesforce"]),

    # ══════════════════════════════════════════════════════════
    # AI / LLM PLATFORMS
    # ══════════════════════════════════════════════════════════
    P("OpenAI API Key (classic)",
      r"(sk-[A-Za-z0-9]{48})",
    ...
    P("Shopify Webhook Secret",
      r"(shpss_[a-fA-F0-9]{32})",
      "confirmed","Shopify webhook secret",["shopify"]),
    P("npm Token",
      r"(npm_[A-Za-z0-9]{36})",
    ...
    P("Vercel Token",
      r"(vc_[A-Za-z0-9]{24,})",
      "confirmed","Vercel access token",["vercel"]),
    P("HashiCorp Vault Service Token",
      r"(hvs\.[A-Za-z0-9_\-+/=]{50,})",
      "confirmed","HashiCorp Vault service token",["vault","secrets"]),
    P("HashiCorp Vault Batch Token",
      r"(hvb\.[A-Za-z0-9_\-]{40,})",
      "confirmed","HashiCorp Vault batch token",["vault","secrets"]),
    P("Doppler Token",
      r"(dp\.(?:st|ct)\.[A-Za-z0-9.]{30,})",
      "confirmed","Doppler secrets token",["doppler","secrets"]),
    P("Databricks Token",
      r"(dapi[a-f0-9]{32})",
      "confirmed","Databricks personal access token",["databricks","cloud"]),
    P("Dynatrace Token",
      r"(dt0[a-z0-9]{2,5}\.[A-Za-z0-9]{8}\.[A-Za-z0-9]{64})",
      "confirmed","Dynatrace API token",["dynatrace","monitoring"]),
    P("PlanetScale Token",
      r"(pscale_tkn_[A-Za-z0-9_]{32,})",
      "confirmed","PlanetScale service token",["planetscale","database"]),
    P("JFrog Artifactory Token",
      r"((?:AKCp[5-9]|AP[\dA-B])[A-Za-z0-9]{60,})",
      "confirmed","JFrog Artifactory API token",["jfrog","devtools"]),
    P("Postman API Key",
      r"(PMAK-[A-Za-z0-9\-]{40,})",
      "confirmed","Postman API key",["postman","devtools"]),
    P("LaunchDarkly SDK Key",
      r"((?:api|sdk)-[a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12})",
      "confirmed","LaunchDarkly API/SDK key",["launchdarkly"]),
    ...
    P("Amplitude API Key",
      r"""(?i)amplitude[_\s]?(?:api[_\s]?)?key\s*[=:]\s*['"`]([a-f0-9]{32})['"`]""",
      "probable","Amplitude project API key",["amplitude","analytics"],3.0),
    P("Segment Write Key",
    ...
    P("HubSpot PAT",
      ...
      "confirmed","HubSpot private app access token",["hubspot","crm"]),
    P("Airtable Token",
      r"(pat[A-Za-z0-9]{14,22}\.[a-f0-9]{64})",
      "confirmed","Airtable personal access token",["airtable"]),
    P("Notion Integration Token",
      r"(secret_[A-Za-z0-9]{40,})",
      "confirmed","Notion integration secret token",["notion"],3.5),
    P("Linear API Key",
      r"(lin_api_[A-Za-z0-9]{30,})",
      "confirmed","Linear API key",["linear"]),
    P("Typeform Token",
      r"(tfp_[A-Za-z0-9]{40,})",
      "confirmed","Typeform personal access token",["typeform"]),
    P("Contentful PAT",
      r"(CFPAT-[A-Za-z0-9_\-]{40,})",
      "confirmed","Contentful personal access token",["contentful","cms"]),
    P("Supabase Key",
      r"""(?i)supabase[_\s]?(?:service[_\s]?role[_\s]?)?(?:key|secret)\s*[=:]\s*['"`]([A-Za-z0-9._\-]{100,})['"`]""",
      "confirmed","Supabase service role key",["supabase","database"],4.0),
    P("Twitch OAuth Token",
      r"(oauth:[a-z0-9]{30,})",
      "confirmed","Twitch OAuth token",["twitch"]),
    ...
    # ══════════════════════════════════════════════════════════
    P("Database DSN",
      r"((?:mysql|postgres(?:ql)?|mongodb(?:\+srv)?|redis|mssql|mariadb|cassandra|elasticsearch)://[A-Za-z0-9_\-]+:[^@\s\"'`]{4,}@[^\s\"'`<>]{4,})",
    ...
    P("innerHTML template literal",
      r"\.innerHTML\s*=\s*`[^`]{0,200}\$\{[^`]{0,100}\}",
      "possible","DOM XSS: innerHTML from template literal",["xss","js"]),
    ...
    P("AWS ARN",
      r"(arn:aws[a-z\-]*:[a-z0-9\-]+:[a-z0-9\-]*:[0-9]{0,12}:[^\s\"'`<>]{4,})",
      "info","AWS resource ARN",["aws","recon"]),
]
...
def main():
    ...
    ap.add_argument("file",            nargs="?",
                    help="File or directory (default: stdin)")
    ...
    if args.list:
        ...
        for p in sorted(PATTERNS, key=lambda x: (SEVMAP[x.sev], x.name)):
            ...
            print(f"  {p.name:<35} {sv:<22} {','.join(p.tags):<30} {p.desc}")
    ...
    if args.file:
        ...
        url_ratio = sum(1 for l in lines
                        if l.startswith(("http://","https://"))) / max(len(lines),1)
    ...
    # fetch
    if args.fetch:
        url_s = [(l,t) for l,t in sources if t.strip().startswith(("http://","https://"))]
        ...
        with ThreadPoolExecutor(max_workers=args.threads) as ex:
            futs = {ex.submit(_f, s): s for s in url_s}
